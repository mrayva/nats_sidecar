...
cxxopts::Options build_cli_options() {
    ...
    options.add_options()
        ...
        ("list-subscriptions-subject", "On-demand subscription-listing request subject", cxxopts::value<std::string>())
...
std::optional<std::string> apply_cli_overrides(config& cfg, const cxxopts::ParseResult& result) {
    ...
    if (result.count("list-subscriptions-subject")) cfg.list_subscriptions_subject = result["list-subscriptions-subject"].as<std::string>();
